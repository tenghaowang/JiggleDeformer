...
MObject jiggleDeformer::mNormalStrength;
MObject jiggleDeformer::mStartFrame;
...
MStatus jiggleDeformer::deform(MDataBlock& pDataBlock,MItGeometry& ItGeom,const MMatrix& localToWorldMatrix,unsigned int geomIndex){
	...
	MDataHandle normalStrengthHandle = pDataBlock.inputValue(mNormalStrength);
	float normalStrength = normalStrengthHandle.asFloat();
	...
	MDataHandle startFrameHandle = pDataBlock.inputValue(mStartFrame);
	int startFrame = startFrameHandle.asInt();
	
	MMatrix worldtoLocalMatrix = localToWorldMatrix.inverse();
	MFloatVectorArray normals;
	if (!biasdirection == 0.0f || normalStrength < 1.0f){
		MObject inputMesh;
	...
	{
		MGlobal::displayWarning("simulation aborted...");
	...
	#pragma omp parallel for private(goalPoint, newPoint, velocity, goalForce, displacement, dampingMagnitude,stiffnessMagnitude)
	//all the calculations are under worldMatrix
	for (int i = 0; i < (int)points.length(); i++){
		...
		if (normalStrength < 1.0f){
			double normalDot = displacement * normals[membershipMap[i]];
			newPoint -= normals[membershipMap[i]] * (displacement * normals[membershipMap[i]]) * (1.0f - normalStrength);
		}
...
MStatus jiggleDeformer::nodeInitialize(){
	...
	mStiffness = nAttr.create("stiffness", "stiffness", MFnNumericData::kFloat, 0.5, &status);
	nAttr.setMin(0.0);
	nAttr.setMax(1.0);
	...
	mDamping = nAttr.create("damping", "damping", MFnNumericData::kFloat, 0.5, &status);
	nAttr.setMin(0.0);
	nAttr.setMax(1.0); 
	nAttr.setKeyable(true);
	...
	mNormalStrength = nAttr.create("normalStrength", "normalStrength", MFnNumericData::kFloat, 1.0, &status);
	nAttr.setMin(0.0);
	nAttr.setMax(1.0);
	nAttr.setKeyable(true);
	addAttribute(mNormalStrength);
	attributeAffects(mNormalStrength, outputGeom);

	mStartFrame = nAttr.create("startFrame", "startFrame", MFnNumericData::kInt, 0.0, &status);
	nAttr.setKeyable(true);
	addAttribute(mStartFrame);
	attributeAffects(mStartFrame, outputGeom);
	...
	MGlobal::displayInfo("Node Initiazed...");
	return MS::kSuccess;
}
